  ...
  <body>
    <div class="wrapper">
        ...
        <div class = "">
            <div class = active-patients>
                <div class = "active-helpers">
                    ...
                    <?php
                  $json = file_get_contents('risk_levels.json');
                  $risk_levels = json_decode($json);
                  ?>
                  <div class="filter-box">
                      <img class="filter" src="svg/filter.svg">
                      <div class="select-box" style="display:none;">
                          <select id="risk_filter" name="risk_level" class="form-control">
                              <option value="" selected="" disabled="">Select risk level</option>
                              <?php
                              foreach($risk_levels as $risk_level) {
                                  echo '<option value="'. $risk_level . '">'. $risk_level . '</option>';
                              }
                              ?>
                          </select>
                      </div>
                ...
                <table>                    
                    ...
                    <tbody>
                    <?php
                        $query = "SELECT * FROM tbl_228_patients";
                        ...
                        if (isset($_GET['risk_level'])) {
                            ...
                            if (strpos($query, 'WHERE') !== false) {
                                $query .= " AND patient_risk = '$riskLevel'";
                            } else {
                                $query .= " WHERE patient_risk = '$riskLevel'";
                            }
                        }
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<div class='actions'><td><div class='actions-flex'><div class='action-1'><img class='action-icon' src='svg/cloud-slash-fill.svg'></div><div class='action-2'><img class='action-icon' src='svg/mic-mute-fill.svg'></div></div></td></div>";
                            echo "</tr>";
                        }
      ...
      <div class="mobile-table-area" style="display:none;">
        <h2><img src="svg/arrow-left.svg">Active Patients</h2>
        ...
        <table class="mobile-table">
          ...
          <?php
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {                 
                          echo "<tr>";
                          echo "<td><div class='status " . $row["patient_risk_desc"] . "'></div></td>"; // Note: You might want to replace 
                          echo "<td><div class='user-image'><img src='svg/person-lines-fill.svg' alt=''></div></td>";
                          echo "<td>";
                          echo "<div class='patients-name'>";
                          echo "<h3>" . $row["patient_firstname"] . " " . $row["patient_lastname"] . "</h3>"; // Replace "name" with your actual 
    ...
    <script src = "js/filter_patients_desk.js"></script>
    <script src = "js/filter_patients.js"></script>
